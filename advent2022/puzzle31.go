...
*/
func depthFirstSearch(start *valve, nzv []*valve, elapsed int, opened map[string]bool) int {

	// Explore all possible path
	max := computeExpurgedPressure(nzv)
	for _, v := range nzv {
		...
		if !opened[v.id] {
			...
			if shouldOpenAt <= MAX_MINUTES {
				...
				opened[v.id] = true
				v.openedAt = shouldOpenAt

				totalGaz := depthFirstSearch(v, nzv, shouldOpenAt, opened)
...
func main() {
	...
	depthFirstSearch(valves["AA"], nonZeroValves, 0, opened)
}
